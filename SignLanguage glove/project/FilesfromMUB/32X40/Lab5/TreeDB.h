 ...
 * Author: Mubarek
 *
 * Created on November 11, 2014, 8:48 PM
 */
...
private:
   TreeNode* root;
   int probesCount;
   bool insert_bst(TreeNode* p, TreeNode* r);
   void printAll( TreeNode*other);
   DBentry*find_helper(TreeNode*other, string name);
   int countActiveHelper( const TreeNode*other, int numberOfActive);
   TreeNode* removeHelper(TreeNode*other, string name);
